...
public extension CityList {
    fileprivate static func create(_ reader : FlatBuffersReader, objectOffset : Offset?) -> CityList? {
        guard let objectOffset = objectOffset else {
            return nil
        }
        if  let cache = reader.cache,
            let o = cache.objectPool[objectOffset] {
            return o as? CityList
        }
        let _result = CityList()
        if let cache = reader.cache {
            cache.objectPool[objectOffset] = _result
        }
        let offset_cityByCountryCode : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 0)
        let length_cityByCountryCode = reader.vectorElementCount(vectorOffset: offset_cityByCountryCode)
        if(length_cityByCountryCode > 0){
            var index = 0
            _result.cityByCountryCode.reserveCapacity(length_cityByCountryCode)
            while index < length_cityByCountryCode {
                if let element = City.create(reader, objectOffset: reader.vectorElementOffset(vectorOffset: offset_cityByCountryCode, index: index)) {
                    _result.cityByCountryCode.append(element)
                }
                index += 1
            }
        }
        let offset_cityByName : Offset? = reader.offset(objectOffset: objectOffset, propertyIndex: 1)
        let length_cityByName = reader.vectorElementCount(vectorOffset: offset_cityByName)
        if(length_cityByName > 0){
            var index = 0
            _result.cityByName.reserveCapacity(length_cityByName)
            while index < length_cityByName {
                if let element = City.create(reader, objectOffset: reader.vectorElementOffset(vectorOffset: offset_cityByName, index: index)) {
                    _result.cityByName.append(element)
                }
                index += 1
            }
        }
...
public extension CityList {
    public static func makeCityList(data : Data,  cache : FlatBuffersReaderCache? = FlatBuffersReaderCache()) -> CityList? {
        let reader = FlatBuffersMemoryReader(data: data, cache: cache)
        return makeCityList(reader: reader)
    }
    public static func makeCityList(reader : FlatBuffersReader) -> CityList? {
...
public struct CityList_Direct<T : FlatBuffersReader> : Hashable, FlatBuffersDirectAccess {
    ...
    public var cityByCountryCode : FlatBuffersTableVector<City_Direct<T>, T> {
    ...
    public var cityByName : FlatBuffersTableVector<City_Direct<T>, T> {
        let offsetList = reader.offset(objectOffset: myOffset, propertyIndex: 1)
        return FlatBuffersTableVector(reader: self.reader, myOffset: offsetList)
    }
    public var hashValue: Int { return Int(myOffset) }
}
public func ==<T>(t1 : CityList_Direct<T>, t2 : CityList_Direct<T>) -> Bool {
...
public extension CityList {
    fileprivate func addToByteArray(_ builder : FlatBuffersBuilder) throws -> Offset {
        ...
        if cityByName.count > 0{
            var offsets = [Offset?](repeating: nil, count: cityByName.count)
            var index = cityByName.count - 1
            while(index >= 0){
                offsets[index] = try cityByName[index].addToByteArray(builder)
                index -= 1
            }
            try builder.startVector(count: cityByName.count, elementSize: MemoryLayout<Offset>.stride)
        ...
        if cityByCountryCode.count > 0{
            var offsets = [Offset?](repeating: nil, count: cityByCountryCode.count)
            var index = cityByCountryCode.count - 1
            while(index >= 0){
                offsets[index] = try cityByCountryCode[index].addToByteArray(builder)
                index -= 1
            }
            try builder.startVector(count: cityByCountryCode.count, elementSize: MemoryLayout<Offset>.stride)
...
public extension City {
    fileprivate static func create(_ reader : FlatBuffersReader, objectOffset : Offset?) -> City? {
        guard let objectOffset = objectOffset else {
            return nil
        }
        if  let cache = reader.cache,
        ...
        _result.countryCode = reader.stringBuffer(stringOffset: reader.offset(objectOffset: objectOffset, propertyIndex: 0))?§
        _result.searchName = reader.stringBuffer(stringOffset: reader.offset(objectOffset: objectOffset, propertyIndex: 1))?§
        _result.name = reader.stringBuffer(stringOffset: reader.offset(objectOffset: objectOffset, propertyIndex: 2))?§
        _result.latitude = reader.get(objectOffset: objectOffset, propertyIndex: 3, defaultValue: 0)
        _result.longitude = reader.get(objectOffset: objectOffset, propertyIndex: 4, defaultValue: 0)
        return _result
    }
}
public struct City_Direct<T : FlatBuffersReader> : Hashable, FlatBuffersDirectAccess {
    fileprivate let reader : T
    fileprivate let myOffset : Offset
    public init?<R : FlatBuffersReader>(reader: R, myOffset: Offset?) {
        guard let myOffset = myOffset, let reader = reader as? T else {
    ...
    public var countryCode : UnsafeBufferPointer<UInt8>? { get { return reader.stringBuffer(stringOffset: reader.offset(objectOffset: myOffset, propertyIndex:0)) } }
    public var searchName : UnsafeBufferPointer<UInt8>? { get { return reader.stringBuffer(stringOffset: reader.offset(objectOffset: myOffset, propertyIndex:1)) } }
    public var name : UnsafeBufferPointer<UInt8>? { get { return reader.stringBuffer(stringOffset: reader.offset(objectOffset: myOffset, propertyIndex:2)) } }
...
public func ==<T>(t1 : City_Direct<T>, t2 : City_Direct<T>) -> Bool {
    return t1.reader.isEqual(other: t2.reader) && t1.myOffset == t2.myOffset
}
public extension City {
    fileprivate func addToByteArray(_ builder : FlatBuffersBuilder) throws -> Offset {
        ...
        let offset2 = try builder.insert(value: name)
        let offset1 = try builder.insert(value: searchName)
        let offset0 = try builder.insert(value: countryCode)
        try builder.startObject(withPropertyCount: 5)
        try builder.insert(value : longitude, defaultValue : 0, toStartedObjectAt: 4)
        try builder.insert(value : latitude, defaultValue : 0, toStartedObjectAt: 3)
...
private func performLateBindings(_ builder : FlatBuffersBuilder) throws {
    for binding in builder.deferedBindings {
        ...
        case let object as CityList: try builder.update(offset: object.addToByteArray(builder), atCursor: binding.cursor)
        case let object as City: try builder.update(offset: object.addToByteArray(builder), atCursor: binding.cursor)
        default: continue
        }
    }
}
...
public extension FlatBuffersReader {
     ...
     */
    private func propertyOffset(objectOffset : Offset, propertyIndex : Int) -> Offset {
...
public final class FlatBuffersBuilder {
    ...
    public func endObject() throws -> Offset {
        ...
        while(index>=0) {
            ...
            let off = Int16(currentVTable[index] != 0 ? Int32(vtableloc) - currentVTable[index] : Offset(0));
            insert(value: off);
            index -= 1
        }
